...
class SegmentedMissionController(Node):
    ...
    def set_segment_parameters(self, desired_vel, xy_tol, yaw_tol):
        """한 구간의 주행 파라미터를 한 번에 설정합니다 (lookahead_dist 포함)."""
        self.get_logger().info(
            f"주행 파라미터 설정: 속도={desired_vel} m/s, XY오차={xy_tol} m, Yaw오차={yaw_tol} rad")
...
def main():
    ...
    # 웨이포인트 정의
    waypoint_0 = make_pose(1.977, -0.018, -1.613)
    waypoint_1 = make_pose(1.880, -6.976, -3.135)
    waypoint_2 = make_pose(0.043, -7.006, 0.040)
    waypoint_3 = make_pose(1.893, -6.963, 1.557)
    waypoint_4 = make_pose(1.988, -0.016, 3.074)
    waypoint_5 = make_pose(0.0, 0.0, 0.0)

    # --- 미션 시작 ---
    
    # 🎯 단계 1: 시작 -> WP0 (도킹)
    node.get_logger().info("====== [단계 1] 웨이포인트 0으로 주행 시작 (도킹) ======")
    params_ok = node.set_segment_parameters(desired_vel=0.3, xy_tol=0.01, yaw_tol=0.05)
    if not params_ok:
        ...
        if not success_step1:
            ...
            # 🚀 단계 2: WP0 -> WP1 (직선 주행)
            node.get_logger().info("====== [단계 2] 웨이포인트 1으로 주행 시작 (직선 주행) ======")
            params_ok = node.set_segment_parameters(desired_vel=1.0, xy_tol=0.01, yaw_tol=0.05)
            if not params_ok:
                ...
                if not success_step2:
                    ...
                    # 🎯 단계 3: WP1 -> WP2 (도킹)
                    node.get_logger().info("====== [단계 3] 웨이포인트 2로 주행 시작 (도킹) ======")
                    params_ok = node.set_segment_parameters(desired_vel=0.3, xy_tol=0.01, yaw_tol=0.05)
                    if not params_ok:
                        ...
                        if not success_step3:
                            ...
                            # 🚀 단계 4: WP2 -> WP3 (직선 주행)
                            node.get_logger().info("====== [단계 4] 웨이포인트 3으로 주행 시작 (직선 주행) ======")
                            params_ok = node.set_segment_parameters(desired_vel=0.3, xy_tol=0.01, yaw_tol=0.05)
                            if not params_ok:
                                ...
                                if not success_step4:
                                    ...
                                    # 🚀 단계 5: WP3 -> WP4 (직선 주행)
                                    node.get_logger().info("====== [단계 5] 웨이포인트 4로 주행 시작 (직선 주행) ======")
                                    params_ok = node.set_segment_parameters(desired_vel=1.0, xy_tol=0.01, yaw_tol=0.05)
                                    if not params_ok:
                                        ...
                                        if not success_step5:
                                            ...
                                            # 🏠 단계 6: WP4 -> WP5 (최종 도킹)
                                            node.get_logger().info("====== [단계 6] 원점(웨이포인트 5)으로 복귀 시작 (최종 도킹) ======")
                                            params_ok = node.set_segment_parameters(desired_vel=0.3, xy_tol=0.01, yaw_tol=0.05)
                                            if not params_ok:
    ...
    node.set_segment_parameters(desired_vel=1.0, xy_tol=0.01, yaw_tol=0.1)
