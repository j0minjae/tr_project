...
#include <tr_localization/Util.h>
#include <tr_localization/Convert.h>
#include <tr_localization/Solver.h>
#include <tr_localization/GridMap.h>
 ...
 *
 */
class TRLocalizationNode : public rclcpp::Node {
public:
	TRLocalizationNode(): Node("tr_localization_node")
	{
		...
		m_map_update_thread = std::thread(&TRLocalizationNode::update_loop, this);

		m_tf_broadcaster = std::make_shared<tf2_ros::TransformBroadcaster>(this);

		m_sub_scan_topic = this->create_subscription<sensor_msgs::msg::LaserScan>(m_scan_topic, rclcpp::SensorDataQoS(), std::bind(&TRLocalizationNode::scan_callback, this, _1));
		m_sub_map_topic = this->create_subscription<nav_msgs::msg::OccupancyGrid>("/map", rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable(), std::bind(&TRLocalizationNode::map_callback, this, _1));
		m_sub_pose_estimate = this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(m_initial_pose, 1, std::bind(&TRLocalizationNode::pose_callback, this, _1));
		...
		m_loc_update_timer = create_wall_timer(
								std::chrono::milliseconds(m_loc_update_time_ms), std::bind(&TRLocalizationNode::loc_update, this));
...
protected:
	...
	std::vector<scan_point_t> convert_scan(const sensor_msgs::msg::LaserScan::SharedPtr scan, const Matrix<double, 4, 4>& odom_to_base)
	{
		...
		try {
			...
			RCLCPP_WARN_STREAM(this->get_logger(), "TRLocalizationNode: lookupTransform(scan->header.frame_id, m_base_frame) failed: " << ex.what());
			return points;
		}
		try {
			...
			RCLCPP_WARN_STREAM(this->get_logger(), "TRLocalizationNode: lookupTransform(m_base_frame, m_odom_frame) failed: " << ex.what());
			return points;
		}
...
// Here
	void loc_update()
	{
		...
		try {
			...
			RCLCPP_WARN_STREAM(this->get_logger(), "TRLocalizationNode: lookup Transform(m_base_frame, m_odom_frame) failed: " << ex.what());
			return;
		}
		...
		{
			RCLCPP_WARN_STREAM(this->get_logger(),"TRLocalizationNode: Number of points too low: " << points.size());
			return;
		}
    ...
    if(m_broadcast_info == true) {
      if(update_counter++ % 10 == 0) {
      	RCLCPP_INFO_STREAM(this->get_logger(),  "TRLocalizationNode: score=" << float(best_score) << ", grad_uvw=[" << float(grad_std_uvw[0]) << ", " << float(grad_std_uvw[1])
	...
	{
		{
			...
			if(pose->header.frame_id != m_map_frame) {
				RCLCPP_WARN_STREAM(this->get_logger(), "TRLocalizationNode: Invalid pose estimate frame");
				return;
			}
			...
			RCLCPP_INFO_STREAM(this->get_logger(), "TRLocalizationNode: Got new map pose estimate: x=" << map_pose.getOrigin()[0]
	...
	void map_callback(const nav_msgs::msg::OccupancyGrid::SharedPtr ros_map)
	{
		...
		RCLCPP_INFO_STREAM(this->get_logger(), "TRLocalizationNode: Got new map with dimensions " << ros_map->info.width << " x " << ros_map->info.height
	...
	void update_loop()
	{
		RCLCPP_WARN_STREAM(this->get_logger(),"TRLocalizationNode: update_map() failed:");
...
int main(int argc, char** argv)
{
	...
	try {
		auto nh = std::make_shared<TRLocalizationNode>() ;
		rclcpp::spin(nh);
	}
	catch(const std::exception& ex) {
		std::cout<<"TRLocalizationNode: " << ex.what() << std::endl;
		return -1;
	}
